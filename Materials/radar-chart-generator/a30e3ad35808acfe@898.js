...
html`
  ...
  main.variable(observer("download")).define("download", ["d3","domToImage","fileSaver"], function(d3,domToImage,fileSaver){return(
d3.select('#download').on('click', () => {
  let size = d3.select("#chart").node().getBoundingClientRect(),
      content = d3.select("#chart").html(),
      cloneId = `clone-${Math.random()}`,
  ...
  main.variable(observer("style")).define("style", ["html","chartConfig"], function(html,chartConfig){return(
html`<style>
  html, body {
    font-size: 1.0rem;
  }
  ...
  main.variable(observer("sampleData")).define("sampleData", function(){return(
[
  {
    "key": "Group 1",
    "values": [
      {
        "axis": "Axis 1",
        "value": 0.11016044441435802
      },
      {
        "axis": "Axis 2",
        "value": 0.8696647550791439
      },
      {
        "axis": "Axis 3",
        "value": 0.677354149502813
      },
      {
        "axis": "Axis 4",
        "value": 0.2767470130303311
      }
    ]
  },
  {
    ...
    "values": [
      {
        "axis": "Axis 1",
        "value": 0.764850754499599
      },
      {
        "axis": "Axis 2",
        "value": 0.4101102267907162
      },
      {
        "axis": "Axis 3",
        "value": 0.08409747421414293
      },
      {
        "axis": "Axis 4",
        "value": 0.20108202752033222
      }
    ]
  },
  {
    ...
    "values": [
      {
        "axis": "Axis 1",
        "value": 0.24029852239131788
      },
      {
        "axis": "Axis 2",
        "value": 0.8898237856143538
      },
      {
        "axis": "Axis 3",
        "value": 0.34252864582381815
      },
      {
        "axis": "Axis 4",
        "value": 0.6527074665992016
      }
    ]
  },
]
...
function format(data) {
  ...
  data.forEach((d) => {
    ...
    // populate axes
    Object.keys(d).filter((k) => k !== "group").forEach((axis) => {
  ...
  return data.map((o) => ({
    key: o.group,
    values: Object.keys(o).filter((k) => k !== "group").map((axis) => ({
      axis,
      value: parseFloat(o[axis]) / 100
    }))
  }))
}
)});
  main.variable(observer("radar")).define("radar", ["RadarChart"], function(RadarChart){return(
RadarChart()
)});
  main.variable(observer()).define(["d3","radar"], function(d3,radar){return(
d3.select("#chart").call(radar)
)});
  main.variable(observer("chartConfig")).define("chartConfig", ["format","d3","csv"], function(format,d3,csv){return(
{
  ...
  main.variable(observer()).define(["radar","chartConfig"], function(radar,chartConfig){return(
radar
  ...
  main.variable(observer("RadarChart")).define("RadarChart", ["d3","alert","svg"], function(d3,alert,svg){return(
function RadarChart() {
    ...
    selection.each(function() {
      ...
      updateData = function() {
        ...
        update_axis_legends.enter()
          ...
          .on('mouseover', function(d, i, j) {
            if (events.axis_legend.mouseover) events.axis_legend.mouseover(d, i, j);
          })
          .on('mouseout', function(d, i, j) {
            if (events.axis_legend.mouseout) events.axis_legend.mouseout(d, i, j);
          })
        ...
        update_radarArea.enter()
          ...
          .attr("class", function(d) {
            return "radarArea " + d.key.replace(/\W/g, '')
          })
          .attr("d", function(d, i) {
    ...
    _data.forEach(function(e) {
      e.values.forEach(function(d, i) {
        var range = ranges[axes[i]] ? // already started?
          ...
          options.axes.ranges[axes[i]].slice() : [0, 1]; // default
  ...
  function radialCalcs() {
    ...
    radial_calcs.rScale = d3.scale.pow()
      .exponent(2)
      .range([0, radial_calcs.radius])
      .domain([0, radial_calcs.maxValue])
  ...
  //   if (updateDimensions && typeof updateDimensions === 'function')
  ...
  chart.margins = function(value) {
    ...
    for (var k = 0; k < vKeys.length; k++) {
      if (mKeys.indexOf(vKeys[k]) >= 0) options.margins[vKeys[k]] = value[vKeys[k]];
    }
    return chart;
  }
  ...
  function legendMouseover(d, i, self) {
    ...
    //Bring back the hovered over blob
    d3.selectAll(".radarArea." + area.replace(/\W/g, ''))
      .transition().duration(200)
      .style("fill-opacity", function(d, i, j) {
        return options.areas.filter.indexOf(d.key) >= 0 ? 0 : 0.7;
      });
  ...
  function tooltip_show(d, i, self) {
    ...
    var newX = parseFloat(d3.select(self).attr('cx')) - 10;
    var newY = parseFloat(d3.select(self).attr('cy')) - 10;
...
require('d3@^3.5.6')
  ...
  main.variable(observer("legend")).define("legend", ["require"], function(require){return(
require('https://bundle.run/d3-svg-legend@1.3.0')
)});
  main.variable(observer()).define(["d3","legend"], function(d3,legend){return(
d3.legend = legend.legend
)});
  main.variable(observer("domToImage")).define("domToImage", ["require"], function(require){return(
require('https://bundle.run/dom-to-image')
)});
  main.variable(observer("fileSaver")).define("fileSaver", ["require"], function(require){return(
require('https://bundle.run/file-saver')
)});
  return main;
}
